...
<div class="reservation-block ring-1 ring-emerald-deep/5 bg-white/60 p-6 rounded-3xl relative transition-[color,background-color,border-color,box-shadow] duration-300 hover:ring-emerald-deep/10" data-index="__INDEX__">
            ...
            @foreach (($roomTypes ?? \App\Support\RoomCatalog::all()) as $type)
                <button type="button" data-type-value="{{ $type['id'] }}" data-type-title="{{ $type['title'] }}"
                    ...
                    <span class="block px-3 py-2.5">
                        ...
                        <span class="mt-0.5 block text-[11px] font-bold text-palay-800 tabnum">₱{{ number_format($type['price']) }}<span class="font-medium text-stone-500"> / night · sleeps {{ $type['beds'] }}</span></span>
                    </span>
                </button>
            @endforeach
        </div>
    ...
    <div class="mt-4" x-data="{ mealsOpen: false }">
        <button type="button" @click="mealsOpen = !mealsOpen" class="inline-flex items-center gap-1.5 py-2 px-3.5 rounded-xl text-xs font-bold bg-white/60 hover:bg-white text-stone-700 transition-[color,background-color,border-color,box-shadow] cursor-pointer border border-emerald-deep/15">
            ...
            <span class="material-icons text-[14px] ml-1 transition-transform duration-200 text-stone-500" :class="mealsOpen ? 'rotate-180' : ''">expand_more</span>
        </button>
        <div class="mt-2.5"
             x-show="mealsOpen"
