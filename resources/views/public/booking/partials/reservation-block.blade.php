...
<div class="reservation-block ring-1 ring-emerald-deep/5 bg-white/60 p-4 sm:p-6 rounded-3xl relative transition-[color,background-color,border-color,box-shadow] duration-300 hover:ring-emerald-deep/10" data-index="__INDEX__">
    ...
    <div class="flex items-center justify-between mb-5 pb-3 border-b border-emerald-deep/10">
        <div class="flex items-center gap-2.5">
            ...
            {{-- The room this block IS, not the name of the step it sits in.
                 ...
                 which room it was. --}}
            <span class="block-room-name text-xs font-bold text-stone-600 uppercase tracking-[0.18em]">Room</span>
    ...
    <div class="room-style-picker" hidden aria-hidden="true">
        <label class="block text-[10px] font-bold text-stone-500 uppercase tracking-widest mb-2">Choose a Room Style</label>
            ...
            @foreach (($roomTypes ?? \App\Support\RoomCatalog::all()) as $type)
                <button type="button" data-type-value="{{ $type['id'] }}" data-type-title="{{ $type['title'] }}" data-beds="{{ $type['beds'] }}"
                    ...
                    <span class="block px-3 py-2.5">
                        ...
                        <span class="mt-0.5 block text-[11px] font-bold text-palay-800 tabnum">₱{{ number_format($type['price']) }}<span class="block font-medium text-stone-500 sm:inline"> / night · sleeps {{ $type['beds'] }}</span></span>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-4 pt-4 border-t border-emerald-deep/10">
        ...
        <div class="count-field count-field--derived">
            ...
            <div class="stepper flex items-center gap-1.5">
                ...
                <input type="number" name="reservations[__INDEX__][num_guests]" id="res-__INDEX__-num-guests" aria-describedby="res-__INDEX__-guests-note" class="res-num-guests w-full px-2 py-2 bg-white/60 border border-emerald-deep/10 rounded-xl text-sm font-bold text-ink text-center outline-none focus:border-gold/60 focus:ring-2 focus:ring-gold/20 transition-[color,background-color,border-color,box-shadow] [appearance:textfield] [&::-webkit-inner-spin-button]:appearance-none" min="1" placeholder="e.g. 2" required>
        ...
        <div class="count-field">
            ...
            <div class="stepper flex items-center gap-1.5">
                ...
                <input type="number" name="reservations[__INDEX__][num_seniors]" id="res-__INDEX__-num-seniors" aria-describedby="res-__INDEX__-seniors-note" class="res-num-seniors w-full px-2 py-2 bg-white/60 border border-emerald-deep/10 rounded-xl text-sm font-bold text-ink text-center outline-none focus:border-gold/60 focus:ring-2 focus:ring-gold/20 transition-[color,background-color,border-color,box-shadow] [appearance:textfield] [&::-webkit-inner-spin-button]:appearance-none" min="0" value="0">
