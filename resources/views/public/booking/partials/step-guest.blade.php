...
<div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-4">
    ...
    <div>
        <label for="guest_phone_alt" class="font-label block text-xs font-normal text-ink-faint tracking-[0.2em] uppercase mb-1.5">
            Second Contact Number <span class="text-ink-faint font-medium normal-case">(Optional)</span>
        </label>
        <div class="relative flex items-center">
            <x-booking.ui.icon-solid name="phone-volume" class="text-ink-faint absolute left-3.5 text-[18px]" />
            <input type="tel" name="guest_phone_alt" id="guest_phone_alt" value="{{ old('guest_phone_alt', $prefill['guest_phone_alt'] ?? '') }}" class="w-full pl-10 pr-4 py-2.5 rounded-xl border border-emerald-deep/10 bg-cream-warm/60 text-ink text-sm placeholder:text-ink-faint focus:bg-cream-warm focus:border-gold/60 focus:ring-2 focus:ring-gold/20 outline-none transition-[color,background-color,border-color,box-shadow] font-semibold"
                   inputmode="numeric" pattern="^(09|\+639)\d{9}$" placeholder="09xxxxxxxxx" maxlength="13">
        </div>
        <p class="text-[11px] font-medium text-ink-faint mt-1.5">Someone else we can reach if we can't get you.</p>
    </div>
    ...
    <div class="sm:col-span-2 co-subsection">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
                <label for="referred_by" class="font-label block text-xs font-normal text-ink-faint tracking-[0.2em] uppercase mb-1.5">
                    Name
                </label>
                <div class="relative flex items-center">
                    <x-booking.ui.icon-solid name="user" class="text-ink-faint absolute left-3.5 text-[18px]" />
                    <input type="text" name="referred_by" id="referred_by" value="{{ old('referred_by') }}" maxlength="255" required
                           list="referredBySuggestions"
                           class="w-full pl-10 pr-4 py-2.5 rounded-xl border border-emerald-deep/10 bg-cream-warm/60 text-ink text-sm placeholder:text-ink-faint focus:bg-cream-warm focus:border-gold/60 focus:ring-2 focus:ring-gold/20 outline-none transition-[color,background-color,border-color,box-shadow] font-semibold"
                ...
                <p class="text-[11px] font-medium text-ink-faint mt-1.5">The CLSU office or staff member endorsing this stay. Nobody sent you? Say “Booking for myself” and use your own details below.</p>
            </div>
            <div>
                <label for="referred_by_phone" class="font-label block text-xs font-normal text-ink-faint tracking-[0.2em] uppercase mb-1.5">
                    Contact Number
                </label>
                <div class="relative flex items-center">
                    <x-booking.ui.icon-solid name="phone" class="text-ink-faint absolute left-3.5 text-[18px]" />
                ...
                <p class="text-[11px] font-medium text-ink-faint mt-1.5">A number the front desk can ring to confirm the endorsement. An office landline is fine.</p>
            </div>
            <div class="sm:col-span-2">
                <label for="referred_by_purpose" class="font-label block text-xs font-normal text-ink-faint tracking-[0.2em] uppercase mb-1.5">
                    Purpose
                </label>
                <div class="relative flex items-center">
                    <x-booking.ui.icon-solid name="clipboard-list" class="text-ink-faint absolute left-3.5 text-[18px]" />
                ...
                <p class="text-[11px] font-medium text-ink-faint mt-1.5">What the stay is for. This is what the desk reads when your arrangement needs checking.</p>
            </div>
        </div>
    </div>
</div>
...
<div class="co-subsection grid grid-cols-1 sm:grid-cols-2 gap-4">
    <div>
        ...
        <div class="relative flex items-center">
            <x-booking.ui.icon-solid name="clock" class="text-ink-faint absolute left-3.5 text-[18px] pointer-events-none" />
            <select name="arrival_time" id="arrival_time" class="w-full appearance-none pl-10 pr-9 py-2.5 rounded-xl border border-emerald-deep/10 bg-cream-warm/60 text-ink text-sm focus:bg-cream-warm focus:border-gold/60 focus:ring-2 focus:ring-gold/20 outline-none transition-[color,background-color,border-color,box-shadow] font-semibold cursor-pointer">
            ...
            <x-booking.ui.icon-solid name="chevron-down" class="text-ink-faint absolute right-3.5 text-[13px] pointer-events-none" />
        </div>
        <p class="text-[11px] font-medium text-ink-faint mt-1.5">Check-in opens at {{ $checkinTime }}. The front desk is staffed 24/7, so a late arrival is fine. It just helps to know.</p>
    ...
    <div>
        <label class="font-label block text-xs font-normal text-ink-faint tracking-[0.2em] uppercase mb-1.5" for="special_requests">
            Special Requests <span class="text-ink-faint font-medium normal-case">(Optional)</span>
        </label>
        <textarea name="special_requests" id="special_requests" rows="3" maxlength="500"
                  class="w-full px-4 py-2.5 rounded-xl border border-emerald-deep/10 bg-cream-warm/60 text-ink text-sm placeholder:text-ink-faint focus:bg-cream-warm focus:border-gold/60 focus:ring-2 focus:ring-gold/20 outline-none transition-[color,background-color,border-color,box-shadow] font-semibold resize-y"
