...
<section class="mb-8">
    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
        <x-admin.card hoverable="true" title="Total Rooms" icon="hotel">
            <p class="text-3xl font-extrabold text-slate-800 mt-1">{{ $totalRooms }}</p>
        </x-admin.card>
        <x-admin.card hoverable="true" title="Occupied Rooms" icon="book_online">
            <p class="text-3xl font-extrabold text-blue-600 mt-1">{{ $occupiedRooms }}</p>
        </x-admin.card>
        <x-admin.card hoverable="true" title="Under Maintenance" icon="construction">
            <p class="text-3xl font-extrabold text-red-600 mt-1">{{ $maintenanceRooms }}</p>
        </x-admin.card>
        <x-admin.card hoverable="true" title="Cleaning" icon="cleaning_services">
            ...
            <div class="room-card bg-white rounded-2xl border border-gray-100 shadow-sm hover:shadow-md transition-all duration-300 cursor-pointer overflow-hidden relative group"
                ...
                <div class="p-4 text-center flex flex-col items-center h-full justify-between">
                    ...
                    <div class="mt-3.5 w-full">
                        <x-admin.button variant="primary" class="btn-update w-full text-[11px] py-1.5 rounded-xl font-bold" data-id="{{ $room->id }}">
                            Edit
                        </x-admin.button>
                    </div>
                </div>
...
<section class="max-w-xl mb-12">
    <x-admin.card title="Add New Room" icon="add_box">
        <form id="add-room-form" action="{{ route('staff.rooms.store') }}" method="POST" class="space-y-4">
            @csrf
            
            <x-admin.input label="Room Number" name="room_number" id="room-number" placeholder="Enter Room Number" required />
            
            <x-admin.select label="Room Type" name="room_type" id="room-type" required>
            ...
            <x-admin.select label="Wing" name="wing" id="wing" required>
            ...
            <x-admin.input label="Price" name="price" id="price" type="number" readonly step="0.01" />
            
            <div class="pt-2">
                <x-admin.button variant="primary" class="w-full rounded-xl">Add Room</x-admin.button>
            </div>
        </form>
    </x-admin.card>
</section>
    ...
    <div class="modal-content rounded-2xl border-0 overflow-hidden shadow-2xl">
      <form id="roomEditForm">
        ...
        <div class="modal-footer border-t border-gray-100 px-6 py-4 flex gap-2 justify-end">
          <x-admin.button type="button" variant="neutral" class="rounded-xl px-4 py-2" data-bs-dismiss="modal">Cancel</x-admin.button>
          <x-admin.button type="submit" variant="primary" class="rounded-xl px-4 py-2">Save changes</x-admin.button>
        </div>
      </form>
    </div>
  </div>
