...
<body class="theme-boutique antialiased font-sans bg-canvas text-ink flex flex-col min-h-screen selection:bg-gold-soft selection:text-ink">
                    ...
                    @auth
                        <div class="relative" x-data="{ open: false }" @click.away="open = false">
                            ...
                            <div x-show="open"
                                 ...
                                 class="absolute right-0 mt-3 w-60 overflow-hidden rounded-2xl border border-emerald-deep/10 bg-cream-warm py-2 text-ink shadow-[0_24px_60px_-20px_rgba(6,40,30,0.35)] z-50"
                                 style="display: none;"
                            >
    ...
    <div id="mobileDrawer" class="fixed inset-0 z-[60] flex justify-end bg-ink/50 backdrop-blur-sm transition-opacity duration-300 hidden">
        <div class="bg-cream w-80 h-full shadow-2xl p-7 flex flex-col justify-between border-l border-gold/20">
            <div>
    ...
    <!-- Editorial Footer -->
    <footer class="relative overflow-hidden bg-emerald-deep text-cream/85 mt-auto" id="Footer">
        <div class="pointer-events-none absolute inset-x-0 top-0 h-px bg-gold/40"></div>
        <div class="mx-auto max-w-7xl px-6 pt-20 pb-14 md:pt-24">
            <div class="grid gap-12 md:grid-cols-3 md:gap-16">
                ...
                <div class="min-w-0">
                    ...
                    <h2 class="mt-4 font-display text-4xl leading-tight text-cream md:text-5xl">Farmers <span class="italic text-gold">Hostel</span></h2>
                    <p class="text-pretty mt-6 max-w-sm text-sm leading-relaxed text-cream/70">
                    ...
                    <p class="mt-8 text-[11px] uppercase tracking-[0.3em] text-cream/55">CLSU · Science City of Muñoz · Nueva Ecija</p>
                </div>
                ...
                <div class="grid grid-cols-2 gap-8 min-w-0">
                    ...
                    <div class="min-w-0">
                        ...
                        <ul class="space-y-3 text-sm text-cream/85 break-words">
                            ...
                            <li class="text-cream/60">Front desk · 24 / 7</li>
                        </ul>
                    </div>
                </div>
                ...
                <div class="min-w-0">
                    ...
                    <p class="text-pretty text-sm leading-relaxed text-cream/70">A seasonal letter from the farmstead — new rooms, harvests, and a standing invitation to breakfast.</p>
        ...
        <div class="border-t border-cream/10 bg-ink/20">
    ...
    <script>
        document.addEventListener('DOMContentLoaded', function() {
            // Scroll-aware nav: dark glass over the hero, cream once scrolled
            const nav = document.getElementById('siteNav');
            if (nav && nav.dataset.dark === '1') {
                const darkCls = ['border-white/15', 'bg-ink/40', 'text-cream'];
                const lightCls = ['border-emerald-deep/10', 'bg-cream-warm/85', 'text-ink', 'shadow-[0_10px_40px_-16px_rgba(6,40,30,0.25)]'];
                let isDark = true;
                const applyNavState = () => {
                    const wantDark = window.scrollY < 80;
                    if (wantDark === isDark) return;
                    isDark = wantDark;
                    nav.classList.remove(...(wantDark ? lightCls : darkCls));
                    nav.classList.add(...(wantDark ? darkCls : lightCls));
                };
                applyNavState();
