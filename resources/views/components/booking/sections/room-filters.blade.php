...
<div id="roomFilters" {{ $attributes->merge(['class' => 'flex flex-wrap items-center gap-3']) }} role="group" aria-label="Filter rooms by capacity">
    @foreach ($filters as $key => $label)
        <button type="button"
                ...
                class="room-filter-pill press focus-ring rounded-full border border-white/15 bg-white/5 px-5 py-2.5 text-[11px] font-bold uppercase tracking-[0.18em] text-ink/70 transition-all duration-300 cursor-pointer hover:border-gold/60 hover:text-ink {{ $loop->first ? 'active' : '' }}">
            {{ $label }}
        </button>
    @endforeach
</div>
