...
<div class="reservation-block border border-slate-200/70 bg-white p-6 rounded-3xl mb-5 relative shadow-[0_8px_30px_rgba(0,0,0,0.02)] hover:shadow-[0_8px_30px_rgba(0,0,0,0.04)] transition-all duration-300" data-index="__INDEX__">
    <div class="flex items-center justify-between mb-4 pb-3 border-b border-slate-100">
        <div class="flex items-center gap-2.5">
            <span class="w-6 h-6 rounded-lg bg-[#0a4f2d] text-white text-[11px] font-black flex items-center justify-center block-number shadow-xs">1</span>
    ...
    <div class="grid grid-cols-1 md:grid-cols-12 gap-4 items-end">
        <div class="md:col-span-5">
            ...
            <select name="reservations[__INDEX__][room_type]" class="room-type-select w-full px-3 py-2.5 rounded-xl border border-slate-200 bg-white text-slate-800 text-sm focus:border-[#0a4f2d] focus:ring-2 focus:ring-[#0a4f2d]/20 outline-none transition-all cursor-pointer font-bold shadow-xs" required>
        ...
        <div class="md:col-span-4">
            ...
            <div class="relative flex items-center">
                <span class="material-icons text-[#0a4f2d]/70 absolute left-3.5 text-[16px] pointer-events-none">payments</span>
                <input type="hidden" name="reservations[__INDEX__][price_per_night]" class="res-price-hidden">
                <input type="text" class="res-price w-full pl-10 pr-3 py-2.5 rounded-xl border border-slate-200/60 bg-emerald-50/30 text-[#0a4f2d] text-sm outline-none font-black text-center select-none cursor-default pointer-events-none shadow-xs" readonly placeholder="P--">
            </div>
        </div>
    </div>
    ...
    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-4 pt-4 border-t border-slate-100">
        <div>
            ...
            <input type="number" name="reservations[__INDEX__][num_guests]" class="res-num-guests w-full px-3 py-2 bg-white border border-slate-200 rounded-xl text-sm font-semibold text-slate-800 outline-none focus:border-[#0a4f2d] focus:ring-2 focus:ring-[#0a4f2d]/20 transition-all shadow-xs" min="1" placeholder="e.g. 2" required>
        ...
        <div>
            ...
            <input type="number" name="reservations[__INDEX__][num_seniors]" class="res-num-seniors w-full px-3 py-2 bg-white border border-slate-200 rounded-xl text-sm font-semibold text-slate-800 outline-none focus:border-[#0a4f2d] focus:ring-2 focus:ring-[#0a4f2d]/20 transition-all shadow-xs" min="0" value="0">
    ...
    <div class="mt-4" x-data="{ mealsOpen: false }">
        <button type="button" @click="mealsOpen = !mealsOpen" class="inline-flex items-center gap-1.5 py-2 px-3.5 rounded-xl text-xs font-bold bg-white hover:bg-slate-50 text-slate-700 transition-all cursor-pointer border border-slate-200 shadow-xs">
            <span class="material-icons text-[15px] text-[#0a4f2d]">restaurant_menu</span>
         ...
         >
            <div class="border border-slate-200/60 rounded-2xl p-5 bg-slate-50/50 space-y-3.5 shadow-inner">
                <p class="text-[10px] font-black text-slate-400 uppercase tracking-wider flex items-center gap-1.5">
                    <span class="material-icons text-[14px] text-[#0a4f2d]">info</span>
    ...
    <div class="mt-5 pt-4 border-t border-slate-100">
        <p class="text-[11px] text-slate-400 font-medium mb-3 flex items-center gap-1.5">
            <span class="material-icons text-[14px] text-[#0a4f2d]">touch_app</span>
        ...
        <button type="button" class="btn-check-availability inline-flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-bold bg-[#0a4f2d] text-white hover:bg-[#0a4f2d]/90 shadow-[0_4px_12px_rgba(8,78,114,0.15)] hover:shadow-[0_6px_16px_rgba(8,78,114,0.25)] transition-all cursor-pointer">
