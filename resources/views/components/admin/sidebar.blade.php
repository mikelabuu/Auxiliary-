...
<aside id="sidebar" 
  ...
  <div class="h-16 flex items-center gap-2.5 px-5 border-b border-white/10 shrink-0">
    <div class="relative w-9 h-9 rounded-lg bg-gradient-to-br from-clsu-400 to-clsu-700 flex items-center justify-center text-white font-bold text-sm shrink-0 ring-1 ring-white/10">
      FH
      <span class="absolute -top-1 -right-1 w-2.5 h-2.5 rounded-full bg-palay-400 ring-2 ring-clsu-950"></span>
    </div>
    <div class="leading-tight label-fade min-w-0">
      <p class="text-sm font-display font-semibold text-white tracking-tight truncate">Farmers Hostel</p>
      <p class="text-[10px] font-semibold text-palay-400 tracking-[0.2em]">ADMIN PORTAL</p>
    </div>
    <button id="collapseToggle" aria-expanded="false" aria-label="Collapse sidebar" 
            class="hidden lg:flex ml-auto w-6 h-6 shrink-0 items-center justify-center rounded-md text-clsu-300/70 hover:text-white hover:bg-white/10 transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-palay-400/60"
  ...
  <nav id="sidebar-nav" class="flex-1 overflow-y-auto overflow-x-hidden px-3 py-4 space-y-6">
    ...
    <!-- SECTION: MAIN -->
    <div>
      <p class="label-fade px-3 text-[10px] font-bold text-clsu-400/70 tracking-widest mb-1.5 uppercase">Main</p>
      <x-admin.sidebar-link :href="route('staff.dashboard')" :active="request()->routeIs('staff.dashboard')">
        <x-slot name="icon">
          <svg class="w-[18px] h-[18px] text-palay-400 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="3" width="7" height="7" rx="1.5"/><rect x="14" y="3" width="7" height="7" rx="1.5"/><rect x="14" y="14" width="7" height="7" rx="1.5"/><rect x="3" y="14" width="7" height="7" rx="1.5"/></svg>
        </x-slot>
        Dashboard
      ...
      <x-admin.sidebar-link :href="route('staff.reports.index')" :active="request()->routeIs('staff.reports.index')">
        <x-slot name="icon">
          <svg class="w-[18px] h-[18px] text-clsu-400 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75" stroke-linecap="round" stroke-linejoin="round"><line x1="4" y1="20" x2="4" y2="10"/><line x1="12" y1="20" x2="12" y2="4"/><line x1="20" y1="20" x2="20" y2="14"/></svg>
        </x-slot>
        Reports
      </x-admin.sidebar-link>
    </div>
    ...
    <div>
      <p class="label-fade px-3 text-[10px] font-bold text-clsu-400/70 tracking-widest mb-1.5 uppercase">Reservations</p>
      
      <!-- Bookings Dropdown -->
      <x-admin.sidebar-dropdown title="Bookings" :active="$bookingsActive">
        <x-slot name="icon">
          <svg class="w-[18px] h-[18px] text-clsu-400 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75" stroke-linecap="round" stroke-linejoin="round"><path d="M3 9a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2v1.5a1.5 1.5 0 0 0 0 3V16a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-2.5a1.5 1.5 0 0 0 0-3V9z"/></svg>
        </x-slot>
      ...
      <x-admin.sidebar-link :href="route('staff.rooms')" :active="request()->routeIs('staff.rooms')">
        <x-slot name="icon">
          <svg class="w-[18px] h-[18px] text-clsu-400 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75" stroke-linecap="round" stroke-linejoin="round"><path d="M2 18v-6a2 2 0 0 1 2-2h16a2 2 0 0 1 2 2v6"/><path d="M2 18h20"/><path d="M6 10V7a2 2 0 0 1 2-2h3v5"/></svg>
        </x-slot>
        Rooms
      </x-admin.sidebar-link>
    </div>
    ...
    <!-- SECTION: FINANCIALS -->
    <div>
      <p class="label-fade px-3 text-[10px] font-bold text-clsu-400/70 tracking-widest mb-1.5 uppercase">Financials</p>
      
      <x-admin.sidebar-link :href="route('staff.paymentlogs.index')" :active="request()->routeIs('staff.paymentlogs.index')">
        <x-slot name="icon">
          <svg class="w-[18px] h-[18px] text-clsu-400 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75" stroke-linecap="round" stroke-linejoin="round"><rect x="2" y="5" width="20" height="14" rx="2"/><line x1="2" y1="10" x2="22" y2="10"/></svg>
        </x-slot>
        Payments
      </x-admin.sidebar-link>

      <x-admin.sidebar-link :href="route('staff.discounts.index')" :active="request()->routeIs('staff.discounts.*')">
        <x-slot name="icon">
          <svg class="w-[18px] h-[18px] text-clsu-400 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75" stroke-linecap="round" stroke-linejoin="round"><path d="M12 2 2 12l9 9 10-10V2z"/><circle cx="7.5" cy="7.5" r="1.4"/></svg>
        </x-slot>
        Discounts
      </x-admin.sidebar-link>
    </div>
    ...
    <!-- SECTION: ACCOUNTS -->
    <div>
      <p class="label-fade px-3 text-[10px] font-bold text-clsu-400/70 tracking-widest mb-1.5 uppercase">Accounts</p>
      
      <x-admin.sidebar-link :href="route('staff.userrecords.index')" :active="request()->routeIs('staff.userrecords.index')">
        <x-slot name="icon">
          <svg class="w-[18px] h-[18px] text-clsu-400 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75" stroke-linecap="round" stroke-linejoin="round"><circle cx="9" cy="8" r="3.2"/><path d="M2.5 20c0-3.6 2.9-6 6.5-6s6.5 2.4 6.5 6"/><circle cx="17.5" cy="9" r="2.4"/><path d="M15.8 14.3c2.4.4 4.7 2 4.7 5.7"/></svg>
        </x-slot>
        Users
      </x-admin.sidebar-link>
      ...
      <x-admin.sidebar-link :href="route('staff.staffrecords.index')" :active="request()->routeIs('staff.staffrecords.index')">
        <x-slot name="icon">
          <svg class="w-[18px] h-[18px] text-clsu-400 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="7" width="18" height="13" rx="2"/><path d="M8 7V5a2 2 0 0 1 2-2h4a2 2 0 0 1 2 2v2"/></svg>
        </x-slot>
        Staff
      </x-admin.sidebar-link>
    </div>
    ...
    <!-- SECTION: SYSTEM -->
    <div>
      <p class="label-fade px-3 text-[10px] font-bold text-clsu-400/70 tracking-widest mb-1.5 uppercase">System</p>
      
      <x-admin.sidebar-link :href="route('staff.audit.index')" :active="request()->routeIs('staff.audit.index')">
        <x-slot name="icon">
          <svg class="w-[18px] h-[18px] text-clsu-400 shrink-0" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.75" stroke-linecap="round" stroke-linejoin="round"><path d="M9 6h11M9 12h11M9 18h11"/><path d="m3 6 1.3 1.3L6.5 5"/><path d="m3 12 1.3 1.3 2.2-2.3"/><path d="m3 18 1.3 1.3 2.2-2.3"/></svg>
        </x-slot>
        Audit Logs
      </x-admin.sidebar-link>
    </div>
