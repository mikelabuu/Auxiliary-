...
<div {{ $attributes->merge(['class' => 'room-card group/card relative bg-white rounded-2xl border border-stone-200/70 shadow-subtle hover:shadow-card-lg hover:border-clsu-200 transition-all duration-200 cursor-pointer']) }}
    ...
    <div class="absolute top-3 right-2.5 flex items-center gap-1 z-10">
        <button type="button" class="room-edit-btn w-7 h-7 rounded-lg bg-white/95 border border-stone-200 text-stone-400 hover:text-clsu-700 hover:border-clsu-300 hover:bg-clsu-50 flex items-center justify-center transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-palay-400" data-id="{{ $room->id }}" title="Edit room" aria-label="Edit room">
        ...
        <div class="relative">
            <button type="button" class="room-kebab-btn w-7 h-7 rounded-lg bg-white/95 border border-stone-200 text-stone-400 hover:text-clsu-700 hover:border-clsu-300 hover:bg-clsu-50 flex items-center justify-center transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-palay-400" data-id="{{ $room->id }}" title="More actions" aria-label="More actions" aria-haspopup="true" aria-expanded="false">
                <x-admin.icon name="kebab" class="w-3.5 h-3.5" stroke-width="2" />
            </button>
            <div data-kebab-panel class="hidden animate-pop absolute right-0 top-full mt-1.5 w-44 bg-white rounded-xl border border-stone-200/70 shadow-card-lg overflow-hidden z-20 py-1">
                ...
                @foreach($statusMeta as $statusKey => $sm)
                    <button type="button" class="quick-status-btn w-full flex items-center gap-2 px-3.5 py-1.5 text-xs text-stone-600 hover:bg-clsu-50 hover:text-clsu-800 transition-colors" data-status-value="{{ $statusKey }}">
                        ...
                        <x-admin.icon name="check" class="quick-status-check icon w-3 h-3 text-clsu-600 {{ $room->status === $statusKey ? '' : 'invisible' }}" stroke-width="2.5" />
                    </button>
                @endforeach
                ...
                <button type="button" class="room-delete-btn w-full flex items-center gap-2 px-3.5 py-1.5 text-xs text-ember-600 hover:bg-ember-50 transition-colors">
                    <x-admin.icon name="trash" class="w-3.5 h-3.5" stroke-width="2" />
