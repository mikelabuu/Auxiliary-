            ...
            @foreach($events as $event)
                <div class="timeline-item flex gap-3">
                    ...
                    <div class="pb-0.5 min-w-0">
                        ...
                        @if($event['detail'])
                            <p class="text-xs text-muted mt-0.5">{{ $event['detail'] }}</p>
                        @endif
                        <p class="text-2xs text-faint mt-0.5 font-data tabnum">{{ $event['at']->timezone(config('hostel.timezone'))->format('M d, Y · g:i A') }}</p>
                    </div>
                </div>
            @endforeach
        </div>
