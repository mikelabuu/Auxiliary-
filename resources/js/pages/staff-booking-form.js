/**
 * Staff booking form — the tap-to-pick room board, guest assignment cards and
 * live summary shared by the admin Manual Booking screen and the front desk
 * Walk-In screen.
 *
 * This was ~595 lines pasted inline into both Blade views. The copies had
 * already started to drift, so it now lives here once and both pages render
 * <x-staff.booking.form>, which carries the per-page bits as data attributes:
 *
 *   data-available-url        POST endpoint returning the availability payload
 *   data-empty-board-message  shown when the estate has no rooms at all
 *
 * Server state that can't be a plain attribute (the `old()` reservations after
 * a failed validation) rides along in an application/json script tag.
 *
 * No-ops on every page that doesn't render the form.
 */

function initStaffBookingForm() {
    const form = document.querySelector('[data-staff-booking-form]');
    if (!form) return;

    const AVAILABLE_URL = form.dataset.availableUrl;
    const EMPTY_BOARD_MESSAGE = form.dataset.emptyBoardMessage || 'No rooms found.';
    ...
    let OLD_RESERVATIONS = [];
    try {
        OLD_RESERVATIONS = JSON.parse(
            document.getElementById('staff-booking-old-reservations')?.textContent || '[]'
        );
    } catch {
        OLD_RESERVATIONS = [];
    }
    ...
    /* ───────────────────── Real-time availability (Reverb) ─────────────────────
       RoomStatusChanged broadcasts whenever any room's status changes anywhere
       (maintenance, cleaning, a new booking, a check-in). We silently re-fetch
       the board so a room another staffer just closed drops off here
       immediately; fetchAvailableRooms → pruneUnavailableSelections() removes it
       from the current picks and toasts a heads-up. Debounced so a burst of
       changes triggers one refresh. */
    if (window.Echo) {
    ...
    if (typeof window.flatpickr !== 'undefined') {
        const initialOutMin = checkInInput.value
            ...
            : 'today';
        fpOut = window.flatpickr(checkOutInput, {
        ...
        fpIn = window.flatpickr(checkInInput, {
...
if (document.readyState === 'loading') {
    document.addEventListener('DOMContentLoaded', initStaffBookingForm);
} else {
    initStaffBookingForm();
}
